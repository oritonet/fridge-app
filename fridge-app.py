...
def display_items():
    ...
    for idx, (item, info) in enumerate(st.session_state.fridge_items.items()):
        ...
        with col:
            st.markdown(f"""
            <div style="position: relative; width: 64px; height: 64px; margin: auto;">
                <img src="data:image/png;base64,{image_base64}"
                    style="width: 64px; height: 64px; border-radius: 8px; object-fit: contain; background: #fff;" />
                <div style="
                    position: absolute;
                    top: 50%;
                    left: 50%;
...
st.markdown("<h2 style='font-size:19px;'>🧊 冷蔵庫在庫管理アプリ</h2>", unsafe_allow_html=True)
display_items()
